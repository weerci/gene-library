...
-- Created by oxana on 26.01.2010, 14:11:24 --
----------------------------------------------
...
prompt
prompt Creating table HISTORY_GROUP
prompt ============================
prompt
create table MODERN.HISTORY_GROUP
(
  ID   NUMBER(10) not null,
...
alter table MODERN.HISTORY_GROUP
  add constraint PK_HISTORY_GROUP primary key (ID)
  using index 
  tablespace SYSTEM
  pctfree 10
  initrans 2
...
alter table MODERN.HISTORY_ACTION
  add constraint FK_HISTORY_ACTION_GROUP foreign key (GROUP_ID)
  references MODERN.HISTORY_GROUP (ID);
...
prompt
prompt Creating table CARD_KIND
prompt ========================
prompt
create table MODERN.CARD_KIND
(
  ID   NUMBER(10) not null,
  NAME NVARCHAR2(32) not null
)
...
alter table MODERN.CARD_KIND
  add constraint PK_CARD_KIND primary key (ID)
  using index 
  tablespace SYSTEM
  pctfree 10
  initrans 2
...
(
  ...
  HASH       NVARCHAR2(512),
  KIND_ID    NUMBER(10) default 0 not null
)
tablespace SYSTEM
  pctfree 10
  pctused 40
...
alter table MODERN.CARD
  ...
  references MODERN.HISTORY (ID);
alter table MODERN.CARD
  add constraint CARD_KIND_FK foreign key (KIND_ID)
  references MODERN.CARD_KIND (ID);
alter table MODERN.CARD
...
create unique index MODERN.CARD_KIND_UX on MODERN.CARD (KIND_ID, CARD_NUM)
  tablespace SYSTEM
  pctfree 10
  initrans 2
  maxtrans 255
...
alter table MODERN.CARD_ARCH
  ...
  references MODERN.EXPERT (ID);
alter table MODERN.CARD_ARCH
  add constraint CARD_ARCH_KIND_FK foreign key (KIND_ID)
  references MODERN.CARD_KIND (ID);
alter table MODERN.CARD_ARCH
  add constraint CARD_ARCH_ORGAN_FK foreign key (ORG_ID)
  references MODERN.ORGANIZATION (ID);
create unique index MODERN.CARD_ARCH_KIND_UX on MODERN.CARD_ARCH (KIND_ID, CARD_NUM)
  tablespace SYSTEM
  pctfree 10
  initrans 2
  maxtrans 255
...
alter table MODERN.IK2
  add constraint IK2_MVD_FK foreign key (MVD_ID)
...
create or replace package body modern.PKG_CARD is
  ...
  function set_card_history(a_id in number, history_action in number, expert_id in number,
    note in nvarchar2) return number is
      ...
      new_hist_id number;
  begin
    select nvl(max(id), 0) into hist_id  from modern.history where obj_id = a_id;

    select modern.history_seq.nextval into new_hist_id from dual;
    if (hist_id > 0) then
      insert into modern.history(id, expert_id, action_id, DATE_INSERT, note, parent_id, obj_id)
      values (new_hist_id, expert_id, history_action, sysdate, note, hist_id, a_id);
      update modern.card c set c.history_id = new_hist_id where c.id = a_id;
    else
      insert into modern.history(id, expert_id, action_id, DATE_INSERT, note, obj_id)
      values (new_hist_id, expert_id, history_action, sysdate, note, a_id);
    end if;
